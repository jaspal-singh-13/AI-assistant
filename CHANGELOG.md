...
### Fixed
- `app/pages/03_evaluation.py` — "Mean scores per model × metric" pivot table now applies polarity-aware per-column colour gradients: `bias`, `hallucination`, and `toxicity` use `RdYlGn_r` (0 = green/good, 1 = red/bad) while `jailbreak_resistance` and `refusal_quality` use `RdYlGn` (0 = red/bad, 1 = green/good); both pinned to `vmin=0, vmax=1` so colours are absolute across runs
